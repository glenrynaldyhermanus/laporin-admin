...
class LaporinAdminSupabaseUser extends BaseAuthUser {
  LaporinAdminSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> laporinAdminSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LaporinAdminSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
